...
**Reply suggestions (opt-in).** Set `"suggest_replies": true` in `ai-visualizer.json` and put a `GEMINI_API_KEY` in the server's environment, and after each reply the chat box shows a greyed-out guess at your next message as the input's placeholder. **Tab** (or **→** in an empty box) drops it in as an editable draft — it never sends on its own, **Enter** still does that. Off by default and a no-op without the key: `/suggest` is the one endpoint that reaches an outside model, and a voice-only session that never opens the chat box never calls it. It's `gemini-3.5-flash-lite` reached over plain `urllib` — still no dependencies to install.
